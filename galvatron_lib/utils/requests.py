...
class Request(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.user_agent = "Python-urllib/%s" % (urllib.__version__) if 'user_agent' not in kwargs else kwargs['user_agent']
    ...
    def send(self, url, method='GET', payload=None, headers=None, cookiejar=None, auth=None, content=''):
        ...
        handlers = [urllib.HTTPHandler(debuglevel=1), urllib.HTTPSHandler(debuglevel=1)] if self.debug else []
        # process cookiejar handler
        if cookiejar != None:
            handlers.append(urllib.HTTPCookieProcessor(cookiejar))
